		...
		if($conn->query($sql)){
			...
			$sql = "UPDATE escuelas t1
					JOIN (
						...
						cct
						FROM escuelas
						WHERE nivel = '$nivel' AND entidad = '$entidad' AND `promedio_general` IS NOT NULL AND total_evaluados > 0 AND poco_confiables<.1*total_evaluados AND grados % $grado = 0 AND grados > 0
